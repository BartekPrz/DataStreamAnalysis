...
reportFriedman <- function(df, group_name, metric_name, metric_direction=1, make_plot = T){
    ...
    if (make_plot){
        if (SAVE_EPS) {
            setEPS()
            postscript(paste0("images/", group_name, "_", metric_name, "_", "Friedman.svg"), width = 7, height = 3.4)
            par(mar=c(0,0,0,0))
            plot = tryCatch({
    ...
    result_ranks_df <- data.frame("Set"=group_name, "Metric"=metric_name, "OOB"=mean_ranks[1, "OOB"], "UOB"=mean_ranks[1, "UOB"], "OB"=mean_ranks[1, "OB"], "NUOB"=mean_ranks[1, "NUOB"], "NOOB"=mean_ranks[1, "NOOB"], "CD"=cd_value, stringsAsFactors = FALSE)
...
for (metric in metrics){
  ...
  df$Algorithm <- ordered(df$Algorithm, levels=c("OOB", "UOB", "OB", "NUOB", "NOOB"))
  ...
  ggsave(paste0("images/imbalance_plot_", metric, ".svg"), imbalance_plot, width=8, height=8)
  ggsave(paste0("images/imbalance_plot_", metric, ".png"), imbalance_plot, width=8, height=8)
}
```
...
for (metric in metrics){
  ...
  df$Algorithm <- factor(df$Algorithm, levels=c("OOB", "UOB", "OB", "NOOB", "NUOB"))
...
for (metric in metrics){
  data_name <- paste0(metric, "-", suffix,".csv")
  df <- read.csv(paste0("data/", data_name), sep=" ", dec=".") %>% 
    filter(grepl('Borderline', Experiment), !grepl('Join', Experiment), !grepl('\\+', Experiment), 
           !grepl('_', Experiment), !grepl('Move', Experiment), !grepl('Split', Experiment), !grepl('Rare', Experiment))
  rank_df <- reportFriedman(df, metric_name=metric, group_name=group_name)
  total_results_df <- rbind(total_results_df, rank_df)
}
```
...
for (metric in metrics){
  ...
  ggsave(paste0("images/borderline_plot_", metric, ".svg"), borderline_plot, width=5, height=5)
...
for (metric in metrics){
  ...
  ggsave(paste0("images/borderline_plot_", metric, "-post-drift.svg"), borderline_plot, width=5, height=5)
...
for (metric in metrics){
  data_name <- paste0(metric, "-", suffix,".csv")
  df <- read.csv(paste0("data/", data_name), sep=" ", dec=".") %>% 
    filter(!grepl('Borderline', Experiment), !grepl('Join', Experiment), !grepl('Move', Experiment), 
           !grepl('\\+', Experiment), !grepl('_', Experiment), !grepl('Split', Experiment), grepl('Rare', Experiment))
  rank_df <- reportFriedman(df, metric_name=metric, group_name=group_name)
  total_results_df <- rbind(total_results_df, rank_df)
}
```
...
for (metric in metrics){
  ...
  ggsave(paste0("images/rare_plot_", metric, ".svg"), rare_plot, width=5, height=5)
...
for (metric in metrics){
  ...
  ggsave(paste0("images/rare_plot_", metric, "-post-drift.svg"), rare_plot, width=5, height=5)
...
for (metric in metrics){
  ...
  ggsave(paste0("images/join_plot_", metric, ".svg"), join_plot, width=5, height=5)
...
for (metric in metrics){
  data_name <- paste0(metric, "-", suffix,".csv")
  df <- read.csv(paste0("data/", data_name), sep=" ", dec=".") %>% 
    filter(!grepl('Borderline', Experiment), !grepl('Join', Experiment), !grepl('\\+', Experiment), !grepl('_', Experiment),
           grepl('Move', Experiment), !grepl('Split', Experiment), !grepl('Rare', Experiment))
  rank_df <- reportFriedman(df, metric_name=metric, group_name=group_name)
  total_results_df <- rbind(total_results_df, rank_df)
}
```
...
for (metric in metrics){
  ...
  ggsave(paste0("images/Move_plot_", metric, ".svg"), Move_plot, width=5, height=5)
...
for (metric in metrics){
  ...
  ggsave(paste0("images/Split_plot_", metric, ".svg"), Split_plot, width=5, height=5)
...
for (metric in metrics){
  ...
  ggsave(paste0("images/Subclusters_plot_", metric, ".svg"), Split_plot, width=5, height=5)
...
for (metric in metrics){
  data_name <- paste0(metric, "-", suffix,".csv")
  df <- read.csv(paste0("data/", data_name), sep=" ", dec=".") %>% 
    filter(!grepl('\\+', Experiment), !grepl('_', Experiment))
  rank_df <- reportFriedman(df, metric_name=metric, group_name=group_name)
  total_results_df <- rbind(total_results_df, rank_df)
}
```
...
for (metric in metrics){
  ...
  pair_plot <-  ggplot(df, aes(x=Metric, y=reorder(Experiment, -Metric), color=Algorithm, shape=Algorithm)) + 
    ...
    scale_color_manual(values = c("#CC3311", "#009988", "#0077BB", "#EE7733", "#EE3377")) + 
    scale_shape_manual(values = c(16, 15, 17, 11, 4)) + 
    labs(x=metric, y = "Pairs of difficulty factors") + 
    xlim(0.3, 1.0) + 
    theme(legend.position = "none", text = element_text(size=20), axis.text.y = element_text(size=16),
          axis.text = element_text(colour = "black"),
          panel.border = element_rect(linetype = "solid", fill = NA))
  
  ggsave(paste0("images/pair_plot_", metric, ".svg"), pair_plot, width=7, height=10)
...
for (metric in metrics){
  ...
  complex_plot <-  ggplot(df, aes(x=Metric, y=reorder(Experiment, -Metric), color=Algorithm, shape=Algorithm)) + 
    ...
    scale_color_manual(values = c("#CC3311", "#009988", "#0077BB", "#EE7733", "#EE3377")) + 
    scale_shape_manual(values = c(16, 15, 17, 11, 4)) + 
    labs(x=metric, y = "Combinations of difficulty factors") + 
    xlim(0.3, 1.0) + 
    theme(legend.position = "none", text = element_text(size=20), axis.text.y = element_text(size=16),
          axis.text = element_text(colour = "black"),
          panel.border = element_rect(linetype = "solid", fill = NA))
  
  ggsave(paste0("images/complex_plot_", metric, ".svg"), complex_plot, width=10, height=10)
...
```{r}
prettyTable(total_results_df, round_columns = c("OOB", "UOB", "OB", "NOOB", "NUOB", "CD"), round_digits = 2)
```
...
```{r}
real_composition <- read.csv("data/composition.csv")

head(real_composition)
```

```{r}
for (datasetName in unique(real_composition$Dataset)){
  print(datasetName)
  df <- real_composition %>%
    filter(Dataset == datasetName) %>%
    mutate(Example = Iteration*2, Minority.ratio = Minority.ratio*100)
  
  minority_ratio_plot <-  ggplot(df, aes(x=Example, y=Minority.ratio)) + 
    geom_line(color="#cc3311") + 
    theme_classic() + 
    labs(x="Number of processed instances [x1000]", y = "Minority ratio [%]") + 
    theme(legend.position = "none", text = element_text(size=20), axis.text = element_text(colour = "black"),
          panel.border = element_rect(linetype = "solid", fill = NA))
  
  ggsave(paste0("images/", datasetName, "_minority_ratio.svg"), minority_ratio_plot, width=10, height=8)
  ggsave(paste0("images/", datasetName, "_minority_ratio.eps"), minority_ratio_plot, width=10, height=8)
  ggsave(paste0("images/", datasetName, "_minority_ratio.png"), minority_ratio_plot, width=10, height=8)
}
```
...
for (datasetName in unique(real_composition$Dataset)){
  ...
  df <- real_composition %>%
    filter(Dataset == datasetName) %>%
    mutate(Example = Iteration*2, Minority.ratio = Minority.ratio*100) %>%
    gather("ExampleType", "ExampleRatio", Safe:Rare) %>%
    mutate(ExampleType = factor(ExampleType, levels = c("Outlier", "Rare", "Borderline", "Safe"))) %>%
    group_by(Example, ExampleType) %>%
    summarise(n = sum(ExampleRatio)) %>%
    mutate(ExampleTypePercentage = n / sum(n)) 
  
  minority_ratio_plot <-  ggplot(df, aes(x=Example, y=ExampleTypePercentage, fill=ExampleType)) + 
    geom_area() + 
    theme_classic() + 
    scale_fill_manual(values = c("#000000", "#BB5566", "#DDAA33", "#008844")) + 
    labs(x="Number of processed instances [x1000]", y = "Example type percentage [%]", fill="Minority example type") + 
        theme(legend.position = "top", text = element_text(size=20), axis.text = element_text(colour = "black"),
          panel.border = element_rect(linetype = "solid", fill = NA))
  
  ggsave(paste0("images/", datasetName, "_type_ratio.svg"), minority_ratio_plot, width=10, height=8)
  ggsave(paste0("images/", datasetName, "_type_ratio.png"), minority_ratio_plot, width=10, height=8)
  ggsave(paste0("images/", datasetName, "_type_ratio.eps"), minority_ratio_plot, width=10, height=8)
}
```
...
cluster_minority <- function(block, minClusterSize=5, q=0.5) {
  clustering <- apcluster(negDistMat(r=2), block, q=q)

  filtered_exemplars <- vector()
  filtered_clusters <- vector()
  cluster_sizes <- vector()
  
  for(clusterIdx in seq_along(clustering@clusters)){
    if(length(clustering@clusters[[clusterIdx]]) >= minClusterSize) {
      filtered_exemplars <- c(filtered_exemplars, clustering@exemplars[[clusterIdx]])
      filtered_clusters <- c(filtered_clusters, clustering@clusters[clusterIdx])
      cluster_sizes <- c(cluster_sizes, length(clustering@clusters[[clusterIdx]]))
    }
  }
  
  list(clusters=filtered_clusters, exemplars=block[filtered_exemplars, ], cluster_sizes=cluster_sizes)
}
...
for(datasetName in c("amazon", "twitter", "tripadvisor", "pakdd")) {
  exemplars_df <- data.frame()
  df <- readARFF(paste0("../../real-streams/streams/", datasetName, ".arff"))
  # standardize the data 
  df <- cbind(df %>% select(-class) %>% scale %>% replace_na(0), 
              df %>% select(class))
  dfSize = nrow(df)

  i = 1
  while(i < dfSize){
    block <- df[i:min(dfSize, i+step),] %>% filter(class=="1.0") %>% select(-class)
    apclustering <- cluster_minority(block, minClusterSize)
    result_df <- rbind(result_df, data.frame(Dataset=datasetName, 
                                             Step=min(dfSize, i+step-1), 
                                             Clusters=length(apclustering$cluster_sizes)))
    exemplars_df <- rbind(exemplars_df, cbind(step=min(dfSize, i+step-1), apclustering$exemplars, size=apclustering$cluster_sizes))
    i = i + step
  }
  
  write.csv(exemplars_df, paste0(datasetName, "_exemplars.csv"))
}

write.csv(result_df, "clustering.csv")
```
```{r}
clusterings <- read.csv("clustering.csv", row.names = 1)

for (datasetName in unique(clusterings$Dataset)){
  print(datasetName)
  df <- clusterings %>%
    filter(Dataset == datasetName) %>%
    mutate(Example = Step/1000)
  
  cluster_count_plot <-  ggplot(df, aes(x=Example, y=Clusters)) + 
    geom_line(color="#0077bb") + 
    theme_classic() + 
    labs(x="Number of processed instances [x1000]", y = "Number of minority sub-clusters") + 
    theme(legend.position = "none", text = element_text(size=20), axis.text = element_text(colour = "black"),
          panel.border = element_rect(linetype = "solid", fill = NA))
  
  ggsave(paste0("images/", datasetName, "_cluster_count.svg"), cluster_count_plot, width=10, height=8)
  ggsave(paste0("images/", datasetName, "_cluster_count.eps"), cluster_count_plot, width=10, height=8)
  ggsave(paste0("images/", datasetName, "_cluster_count.png"), cluster_count_plot, width=10, height=8)
}
```
...
for(datasetName in c("pakdd", "amazon", "twitter", "tripadvisor")) {
  df <- readARFF(paste0("../../real-streams/streams/", datasetName, ".arff"))
  write.csv(df, paste0(datasetName, ".csv"))
  df <- read.csv(paste0(datasetName, ".csv"), row.names = 1)
  pca <- tryCatch({
      princomp(df %>% select(-class) %>% scale %>% replace_na(0))
    },
    error=function(cond) {
      prcomp(df %>% select(-class) %>% scale %>% replace_na(0))
    })
  
  exemplars_df <- read.csv(paste0(datasetName, "_exemplars.csv"), row.names = 1)
  coords <- exemplars_df %>% select(-step, -size)
  exemplars_pca <- predict(pca, coords)
  exemplars_pca_df <- data.frame(PCA1 = exemplars_pca[,1], 
                                 PCA2 = exemplars_pca[,2], 
                                 step = exemplars_df$step,
                                 size = exemplars_df$size)
  
  exemplar_plot <- ggplot(exemplars_pca_df, aes(x=PCA1, y=PCA2, color=step)) + 
    geom_point(mapping=aes(size=size, alpha=0.85)) + 
    theme_classic() + 
    scale_color_viridis(option = "plasma") + 
    labs(color="Step", size="Size") + 
    guides(alpha = "none") +
    theme(text = element_text(size=20), 
          axis.text = element_text(colour = "black"),
          panel.border = element_rect(linetype = "solid", fill = NA))
  
  ggsave(paste0("images/", datasetName, "_exemplars.svg"), exemplar_plot, width=10, height=8)
  ggsave(paste0("images/", datasetName, "_exemplars.png"), exemplar_plot, width=10, height=8)
  ggsave(paste0("images/", datasetName, "_exemplars.pdf"), exemplar_plot, width=10, height=8)
}
```