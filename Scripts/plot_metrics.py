...
def plot_metrics(input_dir, output_dir):
	...
	for entry in os.scandir(input_dir):
		classifier_name = os.path.splitext(entry.name)[0]
		classifier_evaluation_df = pd.read_csv(entry.path, skiprows=range(1, 1 + args.s), na_values=["?"])
		classifier_evaluation_df = classifier_evaluation_df.dropna()
		classifiers[classifier_name] = classifier_evaluation_df
	...
	for metric in args.metrics:
		...
		if args.d:
			...
			ax.legend(loc='lower center', bbox_to_anchor=(0.0, 1.02, 1.0, 0.2), ncol=6, mode="expand",
		...
		if args.d is None:
			plt.savefig(os.path.join(output_dir, metric + '.eps'), bbox_inches='tight')
		plt.close()
